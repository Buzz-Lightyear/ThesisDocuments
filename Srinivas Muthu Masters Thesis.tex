...
\chapter{Challenges in Collecting the Desired Metrics}
	...
	\section{Challenges}
		...
		\subsection{Updating the Statistics}
			% So far, we've established ways to collect the list of processes in memory and the foreground application. The next step is to figure out a way to update the metrics as and when the user requests new applications. Speed of change between applications varies a lot and there is not inherent mechanism to detect a third party app launch as it is a security issue. So, a background thread obtains the list of processes and the foreground app every second to see if there are any changes in the foreground app. If so, it updates the statistics and writes it to a file. We'll see next why it's important to back this data up. Next section will elaborate on the design of the applications and how the service interacts with the application.
		\subsection{Backing Up the Data}
			%Because our background service is running all the time, it's OOM score increases [44] and therefore it's likely to be killed and it's important not to lose the statistics.
		\subsection{Reading the User's Calendar}
			%Public API available
		\subsection{Parsing the Calendar Information}
			%Mapping from top suggested applications in play store and number of downloads of the application for its use
		\subsection{Addressing Privacy Concerns}
			%Both applications are open source. Only way to obtain data is through the user's consent. No personal data is collected, only cache metrics. Full transparency with volunteers - discussed in POW.
