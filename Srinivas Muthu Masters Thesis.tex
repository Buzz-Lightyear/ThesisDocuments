...
\chapter{Experiment Setup and Application Design}
	...
	\section{Eligibility for Volunteers}
		...
		\begin{itemize}
			...
			\item The volunteer must be willing to install two third party applications on their APMD.
		\end{itemize}
	...
	\section{Process and Duration of Experiment}
		A total of nineteen volunteers signed up and installed the applications. They were divided into two groups:
		...
		The first group comprised of 8 volunteers and they were not monitored over a set duration of time. They installed both applications at some point of time and had them running on their APMD(s). The only stipulation that group one was placed under was that they had to run each application for at least a week although there was no upper bound. There were no constraints on whether they had to run both applications simultaneously. Group two comprised of eleven volunteers. They installed and started both applications simultaneously and had them running for exactly one week. Both groups sent the data from each application and additionally provided the number of  entries they entered in their calendar during the time that {\em ContextAnalyzer} was running. The links to both APK files were posted online [56] and each volunteer downloaded and installed each application as a third-party application. In this context, the term third-party refers to the fact that the Android application was'nt downloaded from the Google Play store. The volunteers were instructed not to manually stop the applications for the duration of the experiment [52].
