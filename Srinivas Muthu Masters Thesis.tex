				...
				\paragraph{Prioritizing Overall Cache}	
					...
					\begin{itemize}
						\item Only few applications are suggested by the context module.
							\subitem For instance, say the user has a Calendar entry that reads {\em Call Mom}. There will be two or at most three applications suggested by the context-module that would be potentially useful to the smart-phone user in the context of calling his mother. This makes it more convenient to target CBP(s) cached by the LRU scheme in case of low memory.
						\item The probability of user requesting an application related to the Calendar event is quite high.
							\subitem This is due to the fact that a recorded event in a user's Calendar is highly likely to occur and the applications needed in this context have a higher chance of being utilized as opposed to CBP(s) cached by the LRU scheme.
							\subitem This is explored further in the future scope section where we discuss the relative probabilities of applications being used in conjunction with a Calendar event. 	
					\end{itemize}  
