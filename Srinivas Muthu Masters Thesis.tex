...
\chapter{Experiment Setup and Application Design}
	...
	\section{Eligibility for Volunteers}
		...
		\begin{itemize}
			...
			\item The volunteer must be willing to install two third party applications on their APMD, each for the duration of one week and an overall duration of two weeks.
		\end{itemize}
	...
	\section{Process and Duration of Experiment}
		...
		The overall duration of the experiment was two weeks. Each volunteer downloaded the first application at the start of the experiment and sent the data displayed by the application at the end of first week. Additionally, they installed the second application at the end of first week and sent the data displayed by the second application at the end of second week. They also sent a snapshot of their calendar events during the second week. The volunteers were given detailed instructions to download both applications. The links to the APK files were posted online and each volunteer downloaded and installed each application as a third-party application. In this context, the term third-party refers to the fact that the Android application was'nt downloaded from the Google Play store. The volunteers were instructed not to manually stop the applications for the duration of the experiment. A total of seventeen volunteers signed up and installed the applications.
		 
	\section{Cache Analyzer Design}
		Each volunteer had to install two applications namely {\em CacheAnalyzer} and {\em ContextAnalyzer}. {\em CacheAnalyzer} collected the metrics for the default scheme that Android employs. Every time the user switched to a new application, it identified whether it was a cache hit or cache miss (using the techniques discusses in Section 3-3) and updated the statistics on the file. Its User Interface (UI) listed the following details:
		
		\begin{itemize}
			\item Cache Hits
			\item Cache Misses
			\item Cache Hit Ratio
			\item Recent Cache Hits/Misses
			\item List of Running Processes
		\end{itemize}  
		
		The {\em Cache Hit Ratio} and {\em List of Running Processes} items were merely added to the user interface for the benefit of the volunteer. They did not in any way contribute towards the data collected. The following snapshot (Refer Figure 4-1) illustrates the actual UI of the {\em CacheAnalyzer} application.
	
		\begin{figure}[h]
			\centering
			\includegraphics[width = 70mm]{images/cacheAnalyzerUI.png}
			\caption[Cache Analyzer UI]{The {\em Recent Cache Hits/Misses} section shows the volunteers whether the applications they clicked resulted in a cache hit or a cache miss. It only displays the previous ten hits/misses, not the entire history. The {\em List of Running Processes} section displays the list of processes in RAM (both active and CBP(s)).}
		\end{figure}
		
		Let's inspect the design of this Android application. {\em CacheAnalyzer} comprises of three main modules:
		...
		\end{itemize}
		 ...
		 The main activity [7] is the heart of the application. It is responsible for launching the application and creating a background service[8]. The background service retrieves the list of processes in memory and the current foreground application. The background service is bound to the activity i.e. it's life-cycle is tied to that of the activity's [52]. It is for this reason that the volunteers are instructed not to forcefully close {\em CacheAnalyzer} for the duration of the experiment as that would destroy the service along with the activity and all metric collection would stop. The process manager module contains a {\em Process} data structure that encapsulates a running process and all of its attributes (name, user identifier, process identifier to list a few). It also contains the algorithm that fetches the list of processes residing in memory. The background service launches a separate thread that invokes this algorithm every second to compute the cache metrics and update the user interface. The following class diagram (Refer Figure 4-2) illustrates the design of {\em CacheAnalyzer}. 
		 
		 \begin{figure}[h]
		 	\centering
		 	\includegraphics[width = 110mm]{images/cacheAnalyzerDesign.png}
		 	\caption[Cache Analyzer Class Diagram]{Cache Analyzer Class Diagram}
		 \end{figure}
		 
	\section{Context Analyzer Design}
		{\em ContextAnalyzer} is similar to {\em CacheAnalyzer} in that they both detect cache hits and misses. While the former does so for the default caching scheme, the latter computes the metrics for the Hybrid scheme. {\em ContextAnalyzer} reads the user's calendar and produces a list of applications that the user is likely to use. For the sake of simplicity, let's refer to this list as the calendar list. Similar to {\em CacheAnalyzer}, it also reads the list of processes residing in memory. Let's refer to this list as the memory list. Given this information, every application that the user requests falls in one of four categories:
		
		\begin{itemize}
			\item The application was present in neither list.
			\item The application was present in memory list but not the calendar list.
			\item The application was present in calendar list but not the memory list.
			\item The application was present in both lists. 
		\end{itemize}  
		
		If the application was present in neither list, it results in an overall cache miss. To elaborate, an overall cache miss implies that neither the list of processes in memory (cached by the default LRU caching scheme) nor the list of applications predicted by reading the calendar contained the requested application. If the application was present only in memory list, it results in an overall cache hit and a suggested cache miss i.e. the Hybrid scheme would've still had a hit but it was not present in the list of applications suggested by the calendar list. If the application was present only in the calendar list, it results in an overall cache hit and a suggested-exclusive cache hit, implying that the hit is solely a result of the calendar list's suggestion. This metric is important as it reveals the significance of the list of applications suggested by the calendar list. We'll discuss this further in Chapter 5. Finally, if the application was present in both lists, it results in an overall cache hit and suggested non-exclusive cache hit, implying that the hit would've occurred regardless of whether the application was present in the calendar list or not. The following snapshot (Figure 4-3) illustrates {\em ContextAnalyzer}'s UI.  
		
		\begin{figure}[h]
			\centering
			\includegraphics[width = 130mm]{images/contextAnalyzerUI.png}
			\caption[Context Analyzer UI]{The images to the left, middle and right represent the top, middle and bottom portions of the UI respectively. Note that the Cache Hits in the Suggested Applications Metrics section is divided into suggested-exclusive and suggested non-exclusive. In addition to the cache metrics, it also displays some supplementary data such as the number of unique applications clicked, phone model, OS version and the number of installed applications. These details are discussed in Chapter 5 and are presented as demographic data. Finally, similar to {\em CacheAnalyzer}, it also displays recent cache hits/misses, memory list and the calendar list for the user's benefit.}
		\end{figure}
		
		{\em ContextAnalyzer}'s design closely resembles that of {\em CacheAnalyzer}. Since they share some functionalities such as obtaining the list of processes in memory, and the foreground application, they share modules as well. {\em ContextAnalyzer} comprises of the following modules:
		...
		Note that the first three modules are common between the two applications. The functionalities of the main activity and the process manager are identical to that of {\em CacheAnalyzer}. The background service has the additional task of obtaining the calendar list from the calendar parser in addition to its responsibilities outlined in the previous application's design. The calendar parser gets the list of calendar events from the calendar reader and produces the calendar list (that the service consumes). The calendar reader queries the user's calendar API and obtains the list of events for the next four hours. The following class diagram (Refer Figure 4-4) illustrates {\em ContextAnalyzer}'s design.
	
		\begin{figure}[h]
			\centering
			\includegraphics[width = 150mm]{images/contextAnalyzerDesign.png}
			\caption[Context Analyzer Class Diagram]{Context Analyzer Class Diagram - Note the similarity to {\em CacheAnalyzer}'s design.}
		\end{figure}
		   

		Both applications were developed using Android Studio [53] and the source code was uploaded on Github [50][51]. The class diagrams were built using Gliffy [54]. The APK files were uploaded on our Github I/O page [55][56].
...
\chapter{Data Analysis and Results}
	...
	\section{Experiment Results}
		...
		\subsection{Correlation between Increase in Hybrid CHR \& Calendar Usage}
		
		\subsection{Correlation between Applications Used \& Increase in Hybrid CHR}
