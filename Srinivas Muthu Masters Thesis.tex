...
%			 The speed of change between applications varies a lot and there is not inherent mechanism to detect a third party app launch as it is a security issue. So, a background thread obtains the list of processes and the foreground app every second to see if there are any changes in the foreground app. If so, it updates the statistics and writes it to a file. We'll see next why it's important to back this data up. Next section will elaborate on the design of the applications and how the service interacts with the application.
